<template>
  <b-modal
    id="report_inventory_new"
    @show="initReportInventoryImport"
    size="xl"
    hide-footer
            ...
            <template v-if="inventoryData.materials.length > 0">
              <tr v-for="(inventoryM, key) in inventoryData.materials" :key="key">
                ...
                <td>
                  <select
                    v-if="materials"
                    v-model="inventoryM.material"
                    class="td-select td-select-material"
                    ref="material"
                    @mousedown="_handleSelectMaterialFocus(key)"
                    @change="_handleSelectMaterialChange(key)"
                  >
                    <option disabled selected :value="null">Chọn nguyên vật liệu</option>
                    <template v-for="(material, mkey) in materials">
                      <option
                        v-if="!material.isSelected"
                        :value="material"
                        :key="mkey"
                ...
                <td>{{ (inventoryM.material) ? `${inventoryM.material.unitPrice ? number_with_commas(inventoryM.material.unitPrice) : '- -'}đ/${inventoryM.material.unit ? inventoryM.material.unit : '- -'}` : '' }}</td>
                <td>{{ (inventoryM.material) ? (inventoryM.material.remain ? insertCommasDecimal(inventoryM.material.remain) : 0) : '' }}</td>
                <td>
                  <template v-if="inventoryM.material">
                    <input
                      v-mask="mask_decimal_limit(5)"
                      type="text"
                      v-model="inventoryM.inventoryQuantity"
                      class="td-input"
                      @keyup="_handleInventoryQuantityInput(inventoryM.inventoryQuantity, key)"
                    />
                  </template>
                </td>
                <td>
                  <template
                    v-if="inventoryM.material"
                  >{{ inventoryM.different ? insertCommasDecimal(inventoryM.different) : 0 }}</template>
                </td>
                <td>
                  <template v-if="inventoryM.material">
                ...
                <td>
                  <template v-if="inventoryM.material && inventoryM.inventoryQuantity">
                    <template
                      v-if="inventoryM.material.remain < parseFloat(remove_hyphen(inventoryM.inventoryQuantity))"
                    >Nhập kho</template>
                    <template
                      v-else-if="inventoryM.material.remain > parseFloat(remove_hyphen(inventoryM.inventoryQuantity))"
                    >Xuất kho</template>
                  </template>
                </td>
                <td>
...
export default {
  ...
  methods: {
    ...
    _initImportDate() {
      let curr = new Date();
      this.inventoryData.importDate = `${curr.getFullYear()}-${
        curr.getMonth() < 10 ? `0${curr.getMonth() + 1}` : curr.getMonth() + 1
    ...
    _handleMaterialQuantityChange(key) {
      if (this.inventoryData.materials[key].material !== null) {
        this.inventoryData.materials[key].price =
          Math.ceil(
            remove_hyphen(this.inventoryData.materials[key].material.unitPrice)
          ) *
          Math.ceil(
            remove_hyphen(this.inventoryData.materials[key].quantityImport)
          );
    ...
    _handleInventoryQuantityInput(inventoryQuantity, key) {
      let inventoryQuantityNumber = !check_null(inventoryQuantity)
        ? parseFloat(remove_hyphen(inventoryQuantity))
        : 0;
      if (
        this.inventoryData.materials[key].material.remain >
        inventoryQuantityNumber
      ) {
        this.inventoryData.materials[key].different =
          this.inventoryData.materials[key].material.remain -
          inventoryQuantityNumber;
      } else {
        this.inventoryData.materials[key].different =
    ...
    _handleSaveButtonClick() {
      if (this.$store.getters.getLoader) {
        ...
        if (this.inventoryData.materials.length <= 0) {
          ...
          this.inventoryData.materials.forEach((item, key) => {
            if (item.material === null) {
              ...
              if (check_null(item.inventoryQuantity)) {
                this.formError.list.push(
                  `Số lượng kiểm kê của ${item.material.materialName} không được để trống`
                );
                this.formError.isShow = true;
              }
              if (check_null(item.why) && item.different !== 0) {
                this.formError.list.push(
                  `Nguyên nhân chênh lệch của ${item.material.materialName} không được để trống`
                );
                this.formError.isShow = true;
              }
            }
        ...
        if (!this.formError.isShow) {
          ...
          this.inventoryData.materials.forEach((item) => {
            if (item.material && item.inventoryQuantity) {
              if (
                item.material.remain <
                parseFloat(remove_hyphen(item.inventoryQuantity))
              ) {
                item.solution = 1;
              } else if (
                item.material.remain >
            ...
            requestData.push({
              inventoryCode: !check_null(this.inventoryData.inventoryCode)
                ? this.inventoryData.inventoryCode
                : "",
              inventoryDate: !check_null(this.inventoryData.importDate)
                ? this.inventoryData.importDate
                : "",
              materialId: item.material.materialId,
              materialName: item.material.materialName,
              materialUnit: item.material.unit,
              remainSystem: item.material.remain,
              remainFact: !check_null(item.inventoryQuantity)
                ? parseFloat(remove_hyphen(item.inventoryQuantity))
                : 0,
              quantityDifferent: !check_null(item.different)
                ? item.different
                : 0,
          ...
          this.$store.dispatch("openLoader");
          this.$store
            .dispatch("addNewInventoryMaterial", requestData)
            .then((response) => {
              this.$swal(
                `Tạo mới thành công`,
