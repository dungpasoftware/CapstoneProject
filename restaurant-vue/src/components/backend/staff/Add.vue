  ...
  methods: {
    ...
    _handleAddnewButton() {
      if (this.$store.getters.getLoader) {
        ...
        let requestData = {
          phone: !check_null(this.staffData.phone)
            ? this.staffData.phone.replace(/[-\s\(\)]/g, "")
            : "",
        ...
        if (check_null(this.staffData.password)) {
          ...
          this.staffData.password.length < 8 ||
          this.staffData.password.length > 14
        ) {
          ...
          this.formError.list.push(
            "Mật khẩu và phần nhập lại không trùng khớp"
          );
          this.formError.isShow = true;
        }
        ...
        if (!this.formError.isShow) {
          ...
          this.$store
            .dispatch("addNewStaff", requestData)
            .then((response) => {
              this.$swal(
                "Thành công!",
